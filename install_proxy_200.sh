...
server {
    ...
    location / {
        ...
        proxy_set_header Host 192.168.200.1;
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;

        # Fix redirects from backend dynamically
        proxy_redirect http://192.168.200.1/ http://\$host:8282/;

        # Replace hardcoded URLs inside HTML responses dynamically
        sub_filter 'href="http://192.168.200.1/' 'href="http://\$host:8282/';
        sub_filter 'src="http://192.168.200.1/' 'src="http://\$host:8282/';
        sub_filter_once off;
    }
}
EOF
